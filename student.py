...
def Database():
    ...
    host = "localhost",
    user = "root",
    password = "U$er@my$ql0",
    database = "Store"
)
    cursor = conn.cursor()
    cursor.execute("CREATE TABLE IF NOT EXISTS admin (admin_id int PRIMARY KEY AUTO_INCREMENT NOT NULL, username varchar(20), password varchar(20))")
    cursor.execute("CREATE TABLE IF NOT EXISTS product (product_id int PRIMARY KEY AUTO_INCREMENT NOT NULL, product_name varchar(40), product_qty varchar(20), product_price varchar(20))")
...
def ShowAddUserForm():
    ...
    Adduserform.title("Simple Inventory System/Account Login")
    width = 600
    height = 500
    screen_width = root.winfo_screenwidth()
    screen_height = root.winfo_screenheight()
...
def AddUserForm():
    global lbl_result
    TopLoginForm = Frame(Adduserform, width=600, height=100, bd=1, relief=SOLID)
    TopLoginForm.pack(side=TOP, pady=20)
    lbl_text = Label(TopLoginForm, text="Add User", font=('arial', 18), width=600)
    lbl_text.pack(fill=X)
    MidLoginForm = Frame(Adduserform, width=600)
    MidLoginForm.pack(side=TOP, pady=50)
    lbl_username = Label(MidLoginForm, text="Username:", font=('arial', 25), bd=18)
    lbl_username.grid(row=0)
    lbl_password = Label(MidLoginForm, text="Password:", font=('arial', 25), bd=18)
    ...
    password = Entry(MidLoginForm, textvariable=PASSWORD, font=('arial', 25), width=15, show="*")
    password.grid(row=1, column=1)
    btn_login = Button(MidLoginForm, text="Add", font=('arial', 18), width=30, command=Adduser)
    btn_login.grid(row=2, columnspan=2, pady=20)
...
def AddNew():
    ...
    mycursor.execute("INSERT INTO product (product_name, product_qty, product_price) VALUES(%s,%s,%s)", (PRODUCT_NAME.get(), PRODUCT_QTY.get(), PRODUCT_PRICE.get()))
    PRODUCT_NAME.set("")
    PRODUCT_PRICE.set("")
    PRODUCT_QTY.set("")
    conn.commit()
...
def Adduser(event=None):
    ...
    if USERNAME.get == "" or PASSWORD.get() == "":
        lbl_result.config(text="Please complete the required field!", fg="red")
            ...
            mycursor = conn.cursor()
            mycursor.execute("INSERT INTO admin (username, password) VALUES(%s,%s)", (USERNAME.get(),PASSWORD.get()))
...
filemenu.add_command(label="Add User", command=ShowAddUserForm)
